...
if __name__ == "__main__":
    text_output = []
    llm = LLM(model="lomahony/eleuther-pythia2.8b-hh-sft",
             gpu_memory_utilization=0.30,
             max_model_len=1000,
             enable_prefix_caching=True,
             tensor_parallel_size=1,)
    RM = LLM(model="./HH/Models/RM_BT_iter7",
            gpu_memory_utilization=0.30,
            max_model_len=1000,
            enable_prefix_caching=True,
            tensor_parallel_size=1,
    ...
    # v1 reward models
    v1_reward_tokenizer = AutoTokenizer.from_pretrained("./HH/Models/RM_v1", padding_side='left')
    v1_reward_model = AutoModelForSequenceClassification.from_pretrained("./HH/Models/RM_v1", num_labels=1, torch_dtype=torch.bfloat16).to(device)
