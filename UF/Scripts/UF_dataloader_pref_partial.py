...
if __name__ == "__main__":
    ...
    idxs = np.random.choice(len(raw_train_dataset), size=20000, replace=False)
    raw_train_dataset = raw_train_dataset.select(idxs)
    idxs = np.random.choice(len(raw_eval_dataset), size=256, replace=False)
    raw_eval_dataset = raw_eval_dataset.select(idxs)
    ...
    for row in tqdm(raw_train_dataset):
        ...
        if chosen_len <= 31:
            random_numbers = range(10, chosen_len)
        else:
            random_numbers = random.sample(range(10, chosen_len), 20)
    ...
    # Cache
    train_dataset.save_to_disk(f'{path}/UF_pref_train_partial')
    eval_dataset.save_to_disk(f'{path}/UF_pref_eval_partial')
