...
def test_all(sample_size=50, seed=42, topk=10, max_generation_length=128):
    UF_dataset = load_from_disk("./UF/Data/UF_SFT_eval")
    test_data = UF_dataset.shuffle(seed=seed)
    ...
    with open("./UF/Results/v2.json", "w") as outfile:
        outfile.truncate()
...
if __name__ == "__main__":
    ...
    # base model
    base_tokenizer = AutoTokenizer.from_pretrained("alignment-handbook/zephyr-7b-sft-full")
    base_model = AutoModelForCausalLM.from_pretrained("alignment-handbook/zephyr-7b-sft-full", torch_dtype=torch.float16).to(device1)
    base_model.eval()
    
    # v1 reward models
    v1_reward_tokenizer = AutoTokenizer.from_pretrained("./UF/Models/RM_v1")
    v1_reward_model = AutoModelForSequenceClassification.from_pretrained("./UF/Models/RM_v1", num_labels=1, torch_dtype=torch.float16, cache_dir="./UF/Models/").to(device2)
    v1_reward_model.eval()
    
    # v2 reward models
    v2_reward_tokenizer = AutoTokenizer.from_pretrained("./UF/Models/RM_BT_iter5")
    v2_reward_model = AutoModelForCausalLM.from_pretrained("./UF/Models/RM_BT_iter5", torch_dtype=torch.float16, cache_dir="./UF/Models").to(device2)
