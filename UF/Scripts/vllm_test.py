...
if __name__ == "__main__":
    text_output = []
    llm = LLM(model="alignment-handbook/zephyr-7b-sft-full",
             gpu_memory_utilization=0.30,
             max_model_len=1000,
             enable_prefix_caching=True,
             tensor_parallel_size=1,)
    RM = LLM(model="./UF/Models/RM_BT_iter5",
            gpu_memory_utilization=0.30,
            max_model_len=1000,
            enable_prefix_caching=True,
            tensor_parallel_size=1,
    ...
    # v1 reward models
    v1_reward_tokenizer = AutoTokenizer.from_pretrained("./UF/Models/RM_v1", padding_side='left')
    v1_reward_model = AutoModelForSequenceClassification.from_pretrained("./UF/Models/RM_v1", num_labels=1, torch_dtype=torch.bfloat16).to(device)
