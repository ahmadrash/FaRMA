...
if __name__ == "__main__":
    ...
    base_model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-3.2-1B-Instruct", torch_dtype=torch.float16).to(device1)
    base_model.eval()
    
    # v1 reward models
    v1_reward_tokenizer = AutoTokenizer.from_pretrained("./TLDR/Models/RM_v1")
    v1_reward_model = AutoModelForSequenceClassification.from_pretrained("./TLDR/Models/RM_v1", num_labels=1, torch_dtype=torch.float16, cache_dir="./TLDR/Models/").to(device2)
    v1_reward_model.eval()
    
    # v2 reward models
    v2_reward_tokenizer = AutoTokenizer.from_pretrained("./TLDR/Models/RM_BT_iter2")
    v2_reward_model = AutoModelForCausalLM.from_pretrained("./TLDR/Models/RM_BT_iter2", torch_dtype=torch.float16, cache_dir="./TLDR/Models").to(device2)
    v2_reward_model.eval()

    # test all 
    test_all(sample_size=100, seed=44, topk=10, max_generation_length=64)
